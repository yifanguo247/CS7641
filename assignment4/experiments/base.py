...
class EvaluationStats(object):
    ...
    def __init__(self):
        self.rewards = list()
        self.steps_to_reach_goal = list()
        self.steps_to_reach_goal_mean = 0
    ...
    def add(self, trial_avg_reward, trial_steps_til_reached_goal):
        self.rewards.append(trial_avg_reward)
        self.steps_to_reach_goal.append(trial_steps_til_reached_goal)
        self.compute() # cumulative history

    def compute(self):
        reward_array = np.array(self.rewards)
        self.runs = len(self.rewards) # number of trials
        self.reward_mean = np.mean(reward_array) # average step reward averaged over trials
        ...
        steps_to_reach_goal_array = np.array(self.steps_to_reach_goal)
        self.steps_to_reach_goal_mean = np.mean(steps_to_reach_goal_array)
        self.stat_history.append((
            self.steps_to_reach_goal_mean,
            self.reward_mean,
            self.reward_median,
            self.reward_std,
            self.reward_max,
    ...
    def to_csv(self, file_name):
        self.compute()
        steps_to_reach_goal_means, means, medians, stds, maxes, mins = zip(*self.stat_history)
        with open(file_name, 'w') as f:
            f.write("trial,steps_to_reach_goal, steps_to_reach_goal_mean,reward,mean,median,std,max,min\n")
            writer = csv.writer(f, delimiter=',')
            writer.writerows(zip(range(len(self.rewards)), self.steps_to_reach_goal, steps_to_reach_goal_means, self.rewards, means, medians, stds, maxes, mins))

    def __str__(self):
        return 'steps_to_reach_goal_state_mean: {}, reward_mean: {}, reward_median: {}, reward_std: {}, reward_max: {}, reward_min: {}, runs: {}'.format(
            self.steps_to_reach_goal_mean,
            self.reward_mean,
            self.reward_median,
            self.reward_std,
...
class ExperimentStats(object):
    ...
    def plot_policies_on_map(self, file_name_base, map_desc, color_map, direction_map, experiment, step_preamble,
        ...
        if only_last:
            policy = np.reshape(np.argmax(self.policies[-1], axis=1), map_desc.shape) # converged policy
            v = self.vs[-1].reshape(map_desc.shape) # converged utilities
...
class BaseExperiment(ABC):
    ...
    def run_policy_and_collect(self, solver, policy, num_trials=NUM_TRIALS): # why does it matter what solver runs the policy?
        stats = EvaluationStats()
        for i in range(num_trials): # simulate the policy num_trials times
            step_rewards, num_steps_to_reach_goal = solver.run_policy(policy, self._max_steps)
            stats.add(np.mean(step_rewards), num_steps_to_reach_goal) # add the average step reward for each trial
        stats.compute()
