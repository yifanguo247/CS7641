...
def fetch_mdp_name(file, regexp): # 'output/VI/cliff_walking_grid.csv', re.compile('(.*)_grid\.csv

    search_result = regexp.search(basename(file))
    if search_result is None:
        return False, False

    mdp_name = search_result.groups()[0] # cliff_walking

    return mdp_name, ' '.join(map(lambda x: x.capitalize(), mdp_name.split('_'))) # cliff_walking, Cliff Walking


def process_params(problem_name, params): # VI, {""discount_factor"": 0.4}"

    param_str = '{}'.format(params['discount_factor']) # 0.4
    if problem_name == 'QL':
...
def find_optimal_params(problem_name, base_dir, file_regex): # VI, output/VI, re.compile('(.*)_grid\.csv

    grid_files = glob.glob(os.path.join(base_dir, '*_grid*.csv')) # ['output/VI/cliff_walking_grid.csv', 'output/VI/large_frozen_lake_grid.csv', 'output/VI/frozen_lake_grid.csv']
    logger.info("Grid files {}".format(grid_files))
    best_params = {}
    for f in grid_files:
        mdp, readable_mdp = fetch_mdp_name(f, file_regex) # cliff_walking, Cliff Walking
        ...
        for criterion in ['reward_mean', 'reward_median', 'reward_max']:
            ...
            best = best[best[criterion] == best_value] # get the row with best average step reward over all trials
            if best.shape[0] == 1:
                break
        ...
        params = json.loads(params) # read the discount factor column as a dictionary
        best_index = best.iloc[-1].name

        best_params[mdp] = {
            'name': mdp, # cliff walking
            'readable_name': readable_mdp, # Cliff Walking
            'index': best_index,
            'params': params, # {discount_factor: 0.X}
            'param_str': process_params(problem_name, params) # 0.X
        }

    return best_params


def find_policy_images(base_dir, params): # output/VI, {'VI':  {'name': VI, 'readable_name':  Cliff Walking, 'index': best_index,'params': {discount_factor: 0.X} } 'param_str': 0.X }

    policy_images = {}
    for mdp in params:
        mdp_params = params[mdp]
        fileStart = os.path.join(base_dir, '{}_{}'.format(mdp_params['name'], mdp_params['param_str'])) # VI, 0.X
        image_files = glob.glob(fileStart + '*.png') # VI_0.X*.png
...
def find_data_files(base_dir, params): # # output/VI, {'VI':  {'name': cliff_walking, 'readable_name':  Cliff Walking, 'index': best_index,'params': {discount_factor: 0.X} } 'param_str': 0.X }

    data_files = {}
    for mdp in params:
        mdp_params = params[mdp]
        files = glob.glob(os.path.join(base_dir, '{}_{}.csv'.format(mdp_params['name'], mdp_params['param_str']))) # frozen_lake_0.X.csv
        optimal_files = glob.glob(os.path.join(base_dir, '{}_{}_optimal.csv'.format(mdp_params['name'], mdp_params['param_str'])))
        episode_files = glob.glob(os.path.join(base_dir, '{}_{}_episode.csv'.format(mdp_params['name'], mdp_params['param_str']))) # only for QL
        ...
        data_files[mdp] = {
            'file': files[0], # 'output/VI/frozen_lake_0.9.csv'
            'optimal_file': optimal_files[0] # 'output/VI/frozen_lake_0.9_optimal.csv'
        }
...
def plot_data(data_files, envs, base_dir): # {'VI' : { 'frozen_lake' :  { 'file': 'output/VI/frozen_lake_0.9.csv' 'optimal_file': 'output/VI/frozen_lake_0.9_optimal.csv'  } } }
    ###
    # envs = [
    #     {
    ...
    for problem_name in data_files: # VI
        for mdp in data_files[problem_name]: # frozen_lake
    ...
    #     }
            ...
            mdp_files = data_files[problem_name][mdp] # { 'file': 'output/VI/frozen_lake_0.9.csv' 'optimal_file': 'output/VI/frozen_lake_0.9_optimal.csv'  }

            step_term = 'Steps'
            if problem_name == 'QL':
                step_term = 'Episodes'

            df = pd.read_csv(mdp_files['file']) # 'output/VI/frozen_lake_0.9.csv'

            title = '{}: {} - Time vs {}'.format(env['readable_name'],
                                                 problem_name_to_descriptive_name(problem_name), step_term) # 'Frozen Lake (8x8)': Value Iteration - Time vs Steps
            file_name = os.path.join(os.path.join(base_dir, problem_name), '{}_time.png'.format(mdp)) # output/report/VI/frozen_lake_time.png
            p = plot_time_vs_steps(title, df, xlabel=step_term) # plot the time of each iteration against the iteration number for VI with optimal parameters on this MDP
            p = watermark(p)
            ...
            title = '{}: {} - {} and Delta vs {}'.format(env['readable_name'],
                                                         ...
                                                         reward_term, step_term) # Frozen Lake (8x8): Value Iteration - Value and Delta vs Steps
            file_name = os.path.join(os.path.join(base_dir, problem_name), '{}_reward_delta.png'.format(mdp)) # output/report/VI/frpzem_lake_reward_delta.png
            p = plot_reward_and_delta_vs_steps(title, df, ylabel=reward_term, xlabel=step_term) # plot (cumulative) reward and delta as a function of the iteration number
            p = watermark(p)
            p.savefig(file_name, format='png', dpi=150)
            p.close()
...
def plot_results(envs):
    ...
    best_images = {}
    data_files = {}
    for problem_name in TO_PROCESS: # PI, VI, QL
        logger.info("Processing {}".format(problem_name))

        problem = TO_PROCESS[problem_name] # 'PI': {  'path': 'VI', 'file_regex': re.compile('(.*)_grid\.csv')      }
        problem_path = os.path.join(INPUT_PATH, problem['path']) # INPUT_PATH = 'output' output/VI
        problem_image_path = os.path.join(os.path.join(INPUT_PATH, 'images'), problem['path']) # output/images/VI

        best_params[problem_name] = find_optimal_params(problem_name, problem_path, problem['file_regex']) # {'frozen_lake':  {'name': frozen_lake, 'readable_name':  Cliff Walking, 'index': best_index,'params': {discount_factor: 0.X} } 'param_str': 0.X }
        best_images[problem_name] = find_policy_images(problem_image_path, best_params[problem_name])
        data_files[problem_name] = find_data_files(problem_path, best_params[problem_name]) # find the .csv, _optimal.csv, _episode.csv files for the optimal params on each problem_name MDP
