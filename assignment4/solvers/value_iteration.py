...
class ValueIterationSolver(BaseSolver):
    ...
    def __init__(self, env, discount_factor=DISCOUNT, theta=THETA, verbose=False):
        ...
        self._V = np.zeros(self._env.nS) # utilities

        self._policy = np.zeros([self._env.nS, self._env.nA])
        self._discount_factor = discount_factor
        self._theta = theta # maximum change in the utility of any state in an iteration before VI converges
        self._steps = 0 # iteration
        self._last_delta = theta
        self._step_times = [] # iteration time
