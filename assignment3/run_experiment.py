...
def run_experiment(experiment_details, experiment, timing_key, dim, skiprerun, verbose, timings): #[ExperimentDetails object], experiments.ICAExperiment, 'ICA', None, False, False, {}
    t = datetime.now()
    for details in experiment_details: # ExperimentDetails object
        exp = experiment(details, verbose=verbose) # take the 2nd argument and call its __init__ method

        if not skiprerun:
            logger.info("Running {} experiment: {} ({})".format(timing_key, details.ds_readable_name, dim))
            exp.perform() # perform experiments and write best estimator params to file

        if dim is not None: # skip
            logger.info("Running with dimension {}".format(dim))
            exp.perform_cluster(dim)
    t_d = datetime.now() - t
    timings[timing_key] = t_d.seconds # {'ICA': X seconds} # didn't know python returned modified objects
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bank', action='store_true', help='Run only Bank')
    parser.add_argument('--mnist', action='store_true', help='Run only MNIST dataset')
    ...
    bank_details = {
        'data': loader.BankData(verbose=True, seed=seed),
        'name': 'bank',
        'readable_name': 'Bank',
    }
    mnist_details = {
        'data': loader.MnistData(verbose=verbose, seed=seed),
        'name': 'mnist',
        'readable_name': 'MNIST',
    }
    if args.statlog:
        ...
        datasets.append(bank_details)
    elif args.mnist:
        datasets.append(mnist_details)
