...
def plot_scree(title, df, problem_name, multiple_runs=False, xlabel='Number of Clusters', ylabel=None):
    ...
    x_points = df.index.values # just a counter
    y_points = df[1]
    if multiple_runs:
...
def plot_combined(title, df, data_columns, tsne_data=None, extra_data=None, extra_data_name=None,
    ...
    if extra_data is not None and extra_data_name is not None:
        ex_ax = ax1.twinx()
        ex_ax.plot(extra_data.index.values, extra_data.iloc[:, 0], 'r', linewidth=1,
...
def read_and_plot_problem(problem_name, problem, output_dir): # 'ICA', {'path': 'ICA', 'nn_curve': False, 'multiple_trials': False }, 'output_test2/images/'
    problem['name'] = problem_name # {'path': 'ICA', 'nn_curve': False, 'multiple_trials': False, 'name': 'ICA'}
    problem_path = '{}/{}'.format(input_path, problem['path']) # 'output/ICA'

    if not os.path.exists('{}/{}'.format(output_dir, problem['name'])):
        os.makedirs('{}/{}'.format(output_dir, problem['name']))

    scree_files = glob.glob('{}/*_scree*.csv'.format(problem_path)) # 'output/ICA/*_scree*.csv'
    logger.info("Scree files {}".format(scree_files))
    [read_and_plot_scree(problem, f, output_dir) for f in scree_files] # {'path': 'ICA', 'nn_curve': False, 'multiple_trials': False }, 'output/ICA/bank_scree.csv', 'output_test2/images/'
    ...
    print("where i am looking: {}/clustering/*_logliklihood.csv".format(problem_path))
...
def plot_results():
    for problem_name in to_process:
        logger.info("Processing {}".format(problem_name)) # 'ICA'
        problem = to_process[problem_name] # {''path': 'ICA', 'nn_curve': False, 'multiple_trials': False }

        read_and_plot_problem(problem_name, problem, output_path) # 'ICA', {''path': 'ICA', 'nn_curve': False, 'multiple_trials': False }, 'output_test2/images/'
