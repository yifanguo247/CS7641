...
BEST_NN_PARAMS = {'NN__activation': ['identity'], 'NN__alpha': [1.0],
                  'NN__hidden_layer_sizes': [(100, 10)], 'NN__learning_rate_init': [0.01]}


def run_subexperiment(main_experiment, out, ds=None): # ICAExperiment, ./output/ICA/clustering/', updated_ds
    if not os.path.exists(out):
        os.makedirs(out)

    out = out + '/{}' # ./output/ICA/clustering//{}'
    ...
    clustering_details = ExperimentDetails(
        details.ds if not ds else ds, # WARNING updated_ds replaces ds !!!
        details.ds_name, # bank
        details.ds_readable_name, # Bank
        details.threads, # 1
        details.seed) # # np.random.randint(0, (2 ** 32) - 1)
    ce = clustering.ClusteringExperiment(clustering_details, verbose=main_experiment.get_vebose()) # ExperimentDetails, False

    return ce.perform_for_subexperiment(out, main_experiment) # ./output/ICA/clustering//{}', ICAExperiment
