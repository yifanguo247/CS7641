...
def plot_data(title, data, column_prefixes=None, validate_only=False, nn_curve=False, clear_existing=True,
    ...
    print("column_prefixes: {}".format(column_prefixes))
    if column_prefixes is not None:
        for i, column_prefix in enumerate(column_prefixes):
            if not nn_curve:
                ...
                print("validate_only: {}".format(validate_only))
...
def plot_best_curves(problem_name, files, output_dir, DS_NAME, nn_curve=False):
    ...
    y_label = 'Test Accuracy' if nn_curve else 'Fitness'
    ...
    if nn_curve:
        ...
        print("output_file_name_regex: {}".format(output_file_name_regex))
    else:
