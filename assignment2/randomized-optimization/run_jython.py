...
TEST_BANK = 'data/{}_test.csv'.format('BankData')
TRAIN_BANK = 'data/{}_train.csv'.format('BankData')
VALIDATE_BANK = 'data/{}_validate.csv'.format('BankData')
...
experiment_data = [
    ...
    ([17, 2, 1], 5001, TEST_BANK, TRAIN_BANK, VALIDATE_BANK, 'BankData')
    ]
...
TIMING_FILE = './output_backup/timing.csv'
...
class RunExperiment(Callable):
    ...
    def call(self):
        ...
        try:
            if self.experiment['type'] == 'countones':
                if self.experiment['kind'] == 'rhc':
                    self.result = countones.run_rhc(self.experiment['args'])
                if self.experiment['kind'] == 'sa':
                    self.result = countones.run_sa(*self.experiment['args'])
                if self.experiment['kind'] == 'ga':
                    self.result = countones.run_ga(*self.experiment['args'])
                if self.experiment['kind'] == 'mimic':
                    self.result = countones.run_mimic(*self.experiment['args'])
