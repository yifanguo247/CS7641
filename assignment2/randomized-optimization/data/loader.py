...
class BankData(DataLoader):
    def __init__(self, path='data/bank/bank.csv', verbose=False, seed=1):
        super().__init__(path, verbose, seed)
    ...
    def _preprocess_data(self):
        to_encode = ['job','marital','education','contact','month','poutcome']
        label_encoder = preprocessing.LabelEncoder()
        ...
        df_class = self._data['y']

        self._data = self._data.drop(['y'], axis=1)
        self._data = pd.concat([self._data, df, df_class], axis=1)

        print("data columns: {}".format(self._data.columns))
        print("{}".format(self._data))
        # self._data[['default','housing','loan']] = (self._data[['default','housing','loan']]).astype(int)
